...
// 修改自 @link https://studygolang.com/articles/22530
...
func RsaSignWithSha256(data []byte, keyBytes []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("ParsePKCS8PrivateKey err: %s", err))
	}

	signature, err := rsa.SignPKCS1v15(rand.Reader, privateKey, crypto.SHA256, hashed)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("Error from signing: %s", err))
	}
...
func RsaVerySignWithSha256(data, signData, keyBytes []byte) (bool, error) {
	block, _ := pem.Decode(keyBytes)
	if block == nil {
		return false, errors.New("private key error")
	}
